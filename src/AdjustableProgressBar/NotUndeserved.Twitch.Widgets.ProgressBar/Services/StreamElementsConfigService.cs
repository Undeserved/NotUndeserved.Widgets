...
namespace NotUndeserved.Twitch.Widgets.ProgressBar.Services {
    public class StreamElementsConfigService : IStreamElementsConfigService {
        private readonly HttpClient _httpClient;
        public string SocketUrl { get; private set; } = "wss://sockets.streamlabs.com/socket.io/?EIO=3&transport=websocket";
        public string AccessToken { get; private set; } = string.Empty;

        public StreamElementsConfigService(HttpClient httpClient) {
            _httpClient = httpClient ?? throw new ArgumentNullException(nameof(httpClient));
        }

        public async Task LoadConfigurationAsync() {
            var config = await _httpClient.GetFromJsonAsync<StreamElementsConfig>("config.json");
            if (config != null) {
